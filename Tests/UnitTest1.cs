...
public class Tests {
	...
	public void BranchingDialogueNodes_HaveAllLinesOfText() {
		...
		Assert.Multiple(() => {
			Assert.That(sut.Nodes[0].NodeConnections, Has.Count.EqualTo(2));
			Assert.That(sut.Nodes[0].NodeConnections[0].Destination.Lines[0], Is.EqualTo("You've selected an option."));
			Assert.That(sut.Nodes[0].NodeConnections[1].Destination.Lines[0], Is.EqualTo("You've selected another option."));
		});
	}
	...
	public void BranchingDialogueConnections_HaveQuestionBlock() {
		...
		Assert.Multiple(() => {
			Assert.That(sut.Nodes[0].NodeConnections, Has.Count.EqualTo(2));
			Assert.That(sut.Nodes[0].NodeConnections[0].Name, Is.EqualTo("Select an option"));
			Assert.That(sut.Nodes[0].NodeConnections[1].Name, Is.EqualTo("Select another option"));
		});
	}
}