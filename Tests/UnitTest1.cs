...
public class Tests {
	...
	public void BranchingDialogueNodes_HaveAllLinesOfText() {
		...
		Assert.Multiple(() => {
			Assert.That(sut.Nodes[0].Nodes, Has.Count.EqualTo(2));
			Assert.That(sut.Nodes[0].Nodes[0].Destination.Lines[0], Is.EqualTo("You've selected an option."));
			Assert.That(sut.Nodes[0].Nodes[1].Destination.Lines[0], Is.EqualTo("You've selected another option."));
		});
	}
